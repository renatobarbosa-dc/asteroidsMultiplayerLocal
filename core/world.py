...
class World:
    ...
    def __init__(self, player_count: int = 1) -> None:
        ...
        # Timer para modo multiplayer
        self.multiplayer_timer = (
            float(C.MULTIPLAYER_TIMER_SECONDS)
            if player_count > 1
            else 0.0
        )
    ...
    def _clamp_pos_to_screen(self, pos: Vec) -> Vec:
        ...
        return Vec(
            max(pad, min(w - pad, pos.x)),
            max(pad, min(h - pad, pos.y)),
        )
    ...
    def spawn_black_hole(self) -> None:
        """Cria buraco negro longe das naves."""
        ...
        self.bh_duration = uniform(
            C.BH_DURATION_MIN,
            C.BH_DURATION_MAX,
        )

    def _update_blackhole(self, dt: float) -> None:
        """Atualiza timers e gravidade do buraco negro."""
    ...
    def _apply_commands(
        ...
        for player_id, cmd in commands_by_player_id.items():
            ...
            created_bullets = ship.apply_command(
                cmd,
                dt,
                self.bullets,
            )
    ...
    def _get_nearest_ship_pos(self, from_pos: Vec) -> Vec | None:
        """Retorna posição da nave viva mais próxima."""
    ...
    def _update_multiplayer_mode(self, dt: float) -> None:
        """Gerencia timer e spawning contínuo."""
    ...
    def _end_multiplayer_by_timer(self) -> None:
        """Determina vencedor ao fim do tempo."""
    ...
    def _handle_collisions(self) -> None:
        ...
        for player_id, delta in result.kill_deltas.items():
            if player_id in self.kills:
                ...
                if (
                    self.is_multiplayer
                    and self.kills[player_id] >= C.FLAGS_TO_WIN
                ):
                    self.game_over = True
        ...
        for ship in self.ships.values():
            for powerup in list(self.powerups):
                ...
                if powerup.kind == "double_shot":
                    ...
                    if (
                        self.flags_collected[ship.player_id]
                        >= C.FLAGS_TO_WIN
                    ):
                        self.game_over = True
    ...
    def _ship_die(
        ...
        if self.is_multiplayer:
            base = self.player_spawn_positions.get(
                pid,
                Vec(C.WIDTH / 2, C.HEIGHT / 2),
            )
